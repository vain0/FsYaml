...
module RepresentationTypes =
  ...
  type YamlObject =
    ...
    /// Mapping node
    | Mapping of OrdMap<YamlObject, YamlObject> * Position option
    /// Scalarを特殊化したNullの値
  ...
  module Mapping =
    ...
    let tryFind key (mapping: OrdMap<YamlObject, YamlObject>) = OrdMap.tryPick (pickF key) mapping
    /// <summary>
    /// Mapping内の要素を検索します。
    /// </summary>
    /// <exception cref="System.Collections.Generic.KeyNotFoundException">キーが存在しない場合</exception>
    let find key (mapping: OrdMap<YamlObject, YamlObject>) = OrdMap.pick (pickF key) mapping
