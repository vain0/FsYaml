...
let rec clearPosition = function
  ...
  | Mapping (v, _) ->
    let mapping = v |> OrdMap.toList |> List.map (fun (key, value) -> (clearPosition key, clearPosition value)) |> OrdMap.ofList
...
"""
  ...
  let Nullをパースできる =
    let body input = test {
      let actual = parse input
      let expected = Mapping (OrdMap.ofList [ Scalar (Plain "abc", None), Null None ], None)
      do! actual |> should equal expected
    }
    parameterize {
      case "abc: "
